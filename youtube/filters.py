...
def matches_query(title: str, query: str) -> bool:
    ...
    в заглавието на видеото.

    Проверява САМО заглавието, не и описанието — описанията са прекалено шумни
    (hashtags, генерични инструкции, несвързан текст) и лесно водят до фалшиви
    съвпадения с общи думи от заявката (напр. заявка "само 5 килограма отгоре"
    съвпадаше с рецепта за козунак, защото описанието ѝ случайно съдържаше и
    "килограма", и "отгоре" някъде в текста, без връзка с темата на търсенето).
    """
